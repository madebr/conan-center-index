...
class GolangConanfile(ConanFile):
    ...
    settings = "os", "arch", "compiler", "build_type", "arch_target", "os_target"
    ...
    def _detect_compilers(self):
        with tools.chdir(self.build_folder):
            cmake = CMake(self)
            cmake.configure()
            [cc, cxx] = [l.strip() for l in open("detected_compiler.txt", "r").readlines()]
            return cc, cxx
    ...
    def package_info(self):
        ...
        self.user_info.GOROOT = self._goroot
        self.user_info.PATH = bin_path
